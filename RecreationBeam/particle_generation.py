...
if plot_beam_prop:
    for z in z_key_positions:
        ...
        plot_divergence(states_at_z, title=f"Primary beam at z = {z*100:.1f} cm")
...
YY = []
PY = []
for pid,state in enumerate(secondary_states_at_foil):
    xx = state[0]
    px = state[3]
    yy = state[1]
    py = state[4]
...
# hdivx = axs[0].hist2d(XX, PX, bins=(100,100), range=[[-6e-4,+6e-4],[-3e-3,+3e-3]], rasterized=True)
hdivx = axs[0].hist2d(XX, PX, bins=(100,100), rasterized=False)
axs[0].set_xlabel(r'$x$ [m]')
axs[0].set_ylabel(r'$p_x$ [GeV]')
axs[0].xaxis.set_minor_locator(AutoMinorLocator(10))
axs[0].yaxis.set_minor_locator(AutoMinorLocator(10))
...
fig.suptitle("Positrons at creation", fontsize=16) # Add overall title
...
# Save primary particles at foil
# save_particles_to_hdf5(primary_states_at_foil, 'primary_particles.h5')
